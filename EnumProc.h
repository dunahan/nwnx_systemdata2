...
private:
	unsigned __int64	m_LastCPU;
	DWORD				m_LastTime;
public:
	CFindKillProcess(LPCTSTR modname);
	CFindKillProcess(DWORD pid, HANDLE ph);
	~CFindKillProcess();
	DWORD m_pid; // 0: error, sinon pid du process
	HANDLE m_hp; // NULL: error, sinon handle du process
	DWORD FindProcess(LPCTSTR lpModname);
	BOOL  KillProcess(int iDelay=60000);
	bool IsResponding();
	DWORD GetCPUUsage();
